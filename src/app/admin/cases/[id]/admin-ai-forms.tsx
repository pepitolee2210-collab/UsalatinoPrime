...
import { Sparkles, FileText, Loader2, CheckCircle, Clock, AlertCircle, ChevronDown, ChevronUp, Shield, Users, Copy, Eye } from 'lucide-react'
...
export function AdminAiForms({ caseId, clientName, submissions, minorData }: AdminAiFormsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-500">
        Genera declaraciones juradas, relatos y testimonios usando IA. Llena el formulario con la informacion del cliente durante la llamada de Zoom.
      </p>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
        {AGENT_TABS.map(tab => {
          const Icon = tab.icon
          const count = submissions.filter(s => s.form_type.startsWith(`${tab.id}_`)).length
          return (
            <button
              key={tab.id}
            ...
            >
              <Icon className={`w-4 h-4 ${activeAgent === tab.id ? tab.color : ''}`} />
              {tab.label}
              {count > 0 && (
                <Badge variant="secondary" className="ml-1 text-[10px] h-5 px-1.5">{count}</Badge>
              )}
            </button>
          )
        })}
      ...
      {showForm && activeAgent === 'sij' && (
        <SijForm caseId={caseId} minorData={minorData} generating={generating} setGenerating={setGenerating} generatedDoc={generatedDoc} setGeneratedDoc={setGeneratedDoc} onDone={() => router.refresh()} />
      )}
      {showForm && activeAgent === 'credible_fear' && (
        <CredibleFearForm caseId={caseId} clientName={clientName} generating={generating} setGenerating={setGenerating} generatedDoc={generatedDoc} setGeneratedDoc={setGeneratedDoc} onDone={() => router.refresh()} />
      )}
      {showForm && activeAgent === 'witness' && (
        <WitnessForm caseId={caseId} minorData={minorData} clientName={clientName} generating={generating} setGenerating={setGenerating} generatedDoc={generatedDoc} setGeneratedDoc={setGeneratedDoc} onDone={() => router.refresh()} />
      )}
    </div>
  )
}
...
// === Submissions list ===
function SubmissionsList({ submissions, agent, expandedSub, setExpandedSub }: {
  submissions: AdminAiFormsProps['submissions']
  agent: AgentTab
  expandedSub: string | null
  setExpandedSub: (id: string | null) => void
}) {
  const agentSubs = submissions.filter(s => s.form_type.startsWith(`${agent}_`))
  ...
  return (
    <div className="space-y-2">
      ...
      {agentSubs.map(sub => {
        ...
        const generatedDoc = (sub.form_data as Record<string, unknown>)?.generated_document as string || ''
        const isExpanded = expandedSub === sub.id
        return (
          <Card key={sub.id} className="border-l-4 border-l-[#F2A900]">
            <CardContent className="p-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {new Date(sub.updated_at).toLocaleDateString('es', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {generatedDoc && (
                    <Button size="sm" variant="ghost" onClick={() => setExpandedSub(isExpanded ? null : sub.id)}>
              ...
              {isExpanded && generatedDoc && (
                <div className="mt-3 border rounded-lg overflow-hidden">
                  <div className="bg-gray-50 px-3 py-2 border-b flex justify-between items-center">
                    <span className="text-xs font-medium text-gray-600">
                      Generado con {(sub.form_data as Record<string, unknown>)?.model as string || 'IA'}
                    </span>
                    <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => { navigator.clipboard.writeText(generatedDoc); toast.success('Copiado') }}>
      ...
      {generatedDoc && (
        <div className="mt-4 border-2 border-green-200 rounded-xl overflow-hidden">
          ...
          <div className="bg-gray-50 px-4 py-3 border-t flex justify-end">
            <Button size="sm" variant="outline" onClick={() => { navigator.clipboard.writeText(generatedDoc); toast.success('Copiado al portapapeles') }}>
              <Copy className="w-3.5 h-3.5 mr-1" /> Copiar Documento
            </Button>
          </div>
        </div>
      )}
...
// === SIJ FORM ===
function SijForm({ caseId, minorData, generating, setGenerating, generatedDoc, setGeneratedDoc, onDone }: AdminFormProps & { minorData?: AdminAiFormsProps['minorData'] }) {
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/ai/generate-sij-document', {
        ...
        body: JSON.stringify({ case_id: caseId, agent: 'sij', input: { document_type: selectedType, ...form } }),
      })
      ...
      onDone()
    } catch (err) { toast.error(err instanceof Error ? err.message : 'Error al generar') }
  ...
  return (
    <div className="space-y-5">
      ...
      <fieldset className="space-y-3 p-4 bg-green-50/50 rounded-xl">
        ...
        <div><Label className="text-xs">Hechos clave *</Label><Textarea value={form.key_facts} onChange={e => u('key_facts', e.target.value)} rows={5} placeholder="Describa los hechos del abandono/negligencia..." /><p className="text-[10px] text-gray-400 mt-1">Escriba en español o ingles. El sistema lo limpia y organiza.</p></div>
...
// === CREDIBLE FEAR FORM ===
function CredibleFearForm({ caseId, clientName, generating, setGenerating, generatedDoc, setGeneratedDoc, onDone }: AdminFormProps & { clientName: string }) {
  const [selectedType, setSelectedType] = useState('')
  const [form, setForm] = useState({
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/ai/generate-sij-document', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ case_id: caseId, agent: 'credible_fear', input: { document_type: selectedType, ...form } }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Error')
      setGeneratedDoc(data.document)
...
// === WITNESS FORM ===
function WitnessForm({ caseId, minorData, clientName, generating, setGenerating, generatedDoc, setGeneratedDoc, onDone }: AdminFormProps & { minorData?: AdminAiFormsProps['minorData']; clientName: string }) {
  const [selectedType, setSelectedType] = useState('')
  const [form, setForm] = useState({
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/ai/generate-sij-document', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ case_id: caseId, agent: 'witness', input: { document_type: selectedType, ...form } }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Error')
      setGeneratedDoc(data.document)
