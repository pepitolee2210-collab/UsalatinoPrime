...
export function ContratosView({ basePath }: Props) {
  return (
    <Suspense fallback={<p className="text-sm text-gray-500">Cargando contratos...</p>}>
      <ContratosInner basePath={basePath} />
    </Suspense>
  )
}

function ContratosInner({ basePath }: Props) {
      ...
      {loading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-500 mb-4">No hay contratos guardados aún.</p>
            <Button onClick={handleNewContract} className="bg-[#F2A900] hover:bg-[#D4940A] text-white">
              <Plus className="w-4 h-4 mr-1.5" />
