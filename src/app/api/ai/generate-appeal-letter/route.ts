 ...
 * Body: { case_id: string }
 *
 * Solo admin/paralegal. Encola un job en QStash para generar el Brief / Carta
 * de Apelación con Claude. Devuelve INMEDIATAMENTE el draft_id con status
 * `pending` — el cliente debe hacer polling sobre el endpoint de listado
 * para detectar cuando status pasa a `ready` (o `failed`).
 *
 * Por qué async vía QStash: la llamada a Claude con 4 PDFs nativos toma
 * 60-150s, excediendo el límite de Vercel. El worker no tiene ese límite
 * (corre fuera del request-response cycle del usuario).
 */
export async function POST(request: NextRequest) {
  ...
  // Verificar que no hay un job ya corriendo (idempotencia mínima)
  const { data: inFlight } = await service
    .from('case_appeal_letter_drafts')
    .select('id, status, created_at:generated_at')
    .eq('case_id', caseId)
  ...
  await logActivity({
    caseId,
    category: 'system',
    subcategory: 'system.appeal_letter_enqueued',
    description: `Carta de Apelación encolada (versión ${nextVersion}, draft ${inserted.id})`,
  ...
  return NextResponse.json({
    ...
    status: 'pending',
    message: 'Generación iniciada en segundo plano. Tarda 60-150s.',
  })
}
