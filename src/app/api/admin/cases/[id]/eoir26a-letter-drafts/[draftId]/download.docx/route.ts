...
import { buildEoir26aLetterDocx } from '@/lib/legal/eoir26a-letter-docx'

/**
 * GET /api/admin/cases/[id]/eoir26a-letter-drafts/[draftId]/download.docx
 *
 * Genera un .docx editable a partir del markdown almacenado en
 * `case_eoir26a_letter_drafts.body_md`. Diana lo descarga para ajustarlo en
 * Word antes de presentarlo a la BIA acompañando al Form EOIR-26A.
 */
export async function GET(
  ...
  const docxBytes = await buildEoir26aLetterDocx({
    applicantName,
  ...
  const filename = `Carta-Exoneracion_${caseRow?.case_number ?? id}_v${draft.version}.docx`
