// Inspecciona los AcroForm fields del PDF EOIR-33 (Change of Address / Change of Venue).
// Output: scripts/eoir-33-raw-fields.json + SHA-256 a stdout.
// Uso: node scripts/inspect-eoir-33-fields.mjs
//
// El SHA-256 emitido se hardcodea en src/lib/legal/eoir-33-form-schema.ts.
// Re-correr cada vez que el EOIR publique una nueva revisión del PDF.
...
const PDF_PATH = path.join(repoRoot, 'public', 'forms', 'eoir-33.pdf')
const OUTPUT_PATH = path.join(__dirname, 'eoir-33-raw-fields.json')
...
async function main() {
  ...
  const output = {
    pdf_path: 'public/forms/eoir-33.pdf',
    sha256,
    bytes: bytes.length,
