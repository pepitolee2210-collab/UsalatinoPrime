// Inspecciona los AcroForm fields del PDF FM-SAPCR-100 oficial.
// Output: scripts/sapcr100-raw-fields.json + SHA-256 a stdout.
// Uso: node scripts/inspect-sapcr100-fields.mjs
//
// Re-correr cada vez que TexasLawHelp publique una nueva revision del PDF.
// El SHA-256 emitido se hardcodea en src/lib/legal/sapcr100-form-schema.ts
...
const PDF_PATH = path.join(repoRoot, 'public', 'forms', 'fm-sapcr-100.pdf')
const OUTPUT_PATH = path.join(__dirname, 'sapcr100-raw-fields.json')
...
function checkboxOnValue(field) {
  // Algunos checkboxes en AcroForms usan keys distintos a "Yes" para el estado checked.
  // pdf-lib espera el on-value real para .check() mediante setAcroFieldState.
  try {
...
async function main() {
  ...
  const output = {
    pdf_path: 'public/forms/fm-sapcr-100.pdf',
    sha256,
    bytes: bytes.length,
