// Inspecciona los AcroForm fields del PDF EOIR-26 (Notice of Appeal to the BIA).
// Output: scripts/eoir-26-raw-fields.json + SHA-256 a stdout.
// Uso: node scripts/inspect-eoir-26-fields.mjs
//
// El SHA-256 emitido se hardcodea en src/lib/legal/eoir-26-form-schema.ts.
// Re-correr cada vez que EOIR publique una nueva revisión del PDF.
...
const PDF_PATH = path.join(repoRoot, 'public', 'forms', 'eoir-26.pdf')
const OUTPUT_PATH = path.join(__dirname, 'eoir-26-raw-fields.json')
...
async function main() {
  ...
  const output = {
    pdf_path: 'public/forms/eoir-26.pdf',
    sha256,
    bytes: bytes.length,
