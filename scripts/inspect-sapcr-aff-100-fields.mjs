// Inspecciona los AcroForm fields del PDF FM-SAPCR-AFF-100 (Affidavit for
// Standing of Nonparent) oficial.
// Output: scripts/sapcr-aff-100-raw-fields.json + SHA-256 a stdout.
// Uso: node scripts/inspect-sapcr-aff-100-fields.mjs
//
// Re-correr cada vez que TexasLawHelp publique una nueva revision del PDF.
// El SHA-256 emitido se hardcodea en src/lib/legal/sapcr-aff-100-form-schema.ts
...
const PDF_PATH = path.join(repoRoot, 'public', 'forms', 'fm-sapcr-aff-100.pdf')
const OUTPUT_PATH = path.join(__dirname, 'sapcr-aff-100-raw-fields.json')
...
async function main() {
  ...
  const output = {
    pdf_path: 'public/forms/fm-sapcr-aff-100.pdf',
    sha256,
    bytes: bytes.length,
