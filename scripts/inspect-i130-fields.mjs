// Inspecciona los AcroForm fields del PDF USCIS I-130 oficial (normalizado).
// Output: scripts/i130-raw-fields.json + SHA-256 a stdout.
// Uso: node scripts/inspect-i130-fields.mjs
//
// Re-correr cada vez que USCIS publique una nueva edición del I-130.
// El SHA-256 emitido se hardcodea en src/lib/legal/i130-form-schema.ts
...
const OUTPUT_PATH = path.join(__dirname, 'i130-raw-fields.json')
...
async function main() {
  ...
  const output = {
    pdf_path: 'public/forms/i-130.pdf',
    sha256,
    bytes: bytes.length,
